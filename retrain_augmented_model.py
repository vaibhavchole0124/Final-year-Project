...
print("▶ Starting Augmented Random Forest training script...")
...
data_path = "data/HR_Employee_Attrition_Augmented.csv"
if not os.path.exists(data_path):
    # Fallback if running from root relative to app? No, we created it in data/
...
X = data.drop(columns=['Attrition', 'EmployeeNumber'])
y = data['Attrition']
...
# -----------------------------
# We save into app/models and models/ to be sure (since structure is duplicated)
output_dirs = ["models", "app/models"]

for d in output_dirs:
    os.makedirs(d, exist_ok=True)
    preprocessor_path = os.path.join(d, "preprocessor_rf.pkl")
    model_path = os.path.join(d, "random_forest_model.pkl")
    
    joblib.dump(preprocessor, preprocessor_path)
    joblib.dump(rf_clf, model_path)
    print(f"Saved artifacts to {d}/")

print("\n✅ Metric: Accuracy on Augmented Data")
